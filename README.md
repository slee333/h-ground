...
## 1.1 Set up the project with server, GraphQL, and Prisma
Installing all dependenceis.
...
On terminal:
    yarn init
    yarn add (... all dependenceis)
    
    Example:
    yarn add @babel/{node, preset-env}

Did add all the extensions to keep watch on nodemon.js
Created server.js

## 1.2 Creating GraphQL server

- install dotenv module : read .env file
- Define typeDefs and resolvers for temporary uses
- use GraphQLServer to initiate server
- Server needs port! define port!
- Added callback function to return a message when server starts
- HERE, our server did not recognize "Import". Need babelrc
- Used babel preset env
- Now we created a GraphQL server! Just need to organize things a bit.

## 1.3 Cleaning up some stuffs
- Create api folder and put every necessary things in there
- Create schema.js and merge all typeDefs and resolvers in there. For details, check github commit

* * *

# 2. Intro to Prisma! Create GraphQL API!
...
**Make sure you add prisma.yml file to .gitignore. This file has your secret!**

Now I need to work on datamodel.prisma for my App. Will do this lata.

## To do list

- [X] Create an account
- [ ] Request Secret
- [ ] Confirm Secret (login, via email for demo. Via phone in real practice) 카카오톡 같은 경우 이메일이 아니라 핸드폰을 통해 인증함. 이게 우리도 더 편할 것. 굳이 카카오 프로필을 쓰거나 하지 말고 (이 또한 개인정보 sensitive하니까 카카오랑 똑같은 방식을 취하는 방법도 있음.)
- [ ] like / unlike a thread
- [ ] Search by user
- [ ] Comment on a thread (subthread)
- [ ] Search user by interest (topic of interest)
- [ ] Follow / Unfollow user
- [ ] See full Post
- [ ] Access user from thread
- [ ] Edit my profile
- [ ] Create account
- [ ] Upload and edit threads
- [ ] See the feed